const local_data = [
  {
    ...
    content: [
      ...
      "This is my model car designed from a combination of primitive shapes. The car is largely unimpressive, but as with anything in computer science, the real joy is in the exploration. Behind the scenes, processing is actually redrawing all the shapes 120 times a second - and the animation is still playing flawlessly. With this small glimpse into the fundamentals of CGI technology, I was instantly invested in the sheer possibilities - specifically shaders for game design and so on.",
...

  {
    ...
    content: [
      ...
      "There are a lot of music players so there is no groundbreaking achievement here. But this was a cool way to recreate the olden day gramophone with a modern twist. Plus, it was a chance to explore the idea of modular design in a safe space.",
    ],
...

  {
    ...
    content: [
      "Up until recently, most of my game development projects have steered away from autonomous agents or enemy AI that operate on anything more than predetermined routes and actions. That is until I discovered the navmesh tool and the strikingly simple math behind complex movements.",
      ...
      "I want to draw your attention to the path interception algorithm that brings everything to life. Each character has a velocity reporter that tracks the velocity with respect to real world time. Using the velocity and direction of a player, an enemy character can predict the future position of that player and move to intercept instead of just following linearly.",

      "image",

      "I'm always intrigued by the humble beginnings of complex concepts. It's techniques like these that formed the basis for making opposing gunmen in Call of Duty feel more believable. Feel free to use the algorithm to make your enemies appear smarter.",
    ],
...

  {
    ...
    content: [
      "An FPGA development board is an integrated circuit designed to be programmed after manufacturing. For those who have no idea what that means, FPGA boards are devices that allow you to connect different wires dynamically so you can tell the board to do different things based on the flow of current - and it can do it really fast and efficiently. But before we got to the FPGA board, we started out building these circuits using simple gates.",
      ...
      "This final project was a combination of these FPGA boards and assembly language. It was my first time exploring the cryptic language and using code to affect hardware and I was thoroughly excited for the adventure.",
...

  {
    ...
    title: "Homemade todo list (bon bon appétit)",
    content: [
      "React is genius. It's like a sweet blend of HTML, CSS, and JavaScript. So, I wanted to see how my first react project could showcase these capabilities. I decided to make a dynamic todo list but I think I bit off more than I could chew. The first hurdle was adding new items to the list on the fly. This was pretty straightforward after a got a handle on React's Document Object Model and component-style approach.",

      "image",

      "Then, I had to add functionality to rearrange list items. The problem is that I would have to offset elements, handle mouse movements, set up triggers when list items overlap and smoothly animate list items while performing bounds checking.",

      "image",

      "In hindsight, I was pushing react a bit too far. React by itself is limited when it comes to robust cross component state management. In the next iteration of this todo-list, I'll be using react redux for state management. It would also be better to move away from the class-based componenents and build the app using function-based components.",

      "All in all, I think this was an insightful first expedition into the world of react. In more recent news, this website was built with react. With a better grasp of the react way of thinking, I'm moving on to integrating express and mongoDB to complete my MERN stack skillset.",
    ],
